...
type PostCreateBody = Prisma.Args<typeof prisma.price_plan_requirement, 'create'>['data']
...
export class RequirementPricePlanService {
  ...
  async create(createRequirementPricePlanDto: PostCreateBody) {
    try {
      ...
      this.logger.error(`Error creating requirements for the plan id: ${createRequirementPricePlanDto.price_plan_id} \n\n ${error}`, RequirementPricePlanService.name);
      throw new HttpException("Error creating comapnies", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async findAllRequirements(query: { price_plan_id?: string }) {
    ...
    try {
      ...
      this.logger.error(`Error fetching requirements for this plan id: ${query.price_plan_id} \n\n ${error}`, RequirementPricePlanService.name);
      throw new NotFoundException("Error fetching comapnies");
    }
  }

  async update(id: string, updateRequirementPricePlanDto: Prisma.Price_plan_requirementUpdateInput) {
    try {
      ...
      this.logger.error(`Error updating requirements for this plan id: ${id} \n\n ${error}`, RequirementPricePlanService.name);
      throw new HttpException("Error updating comapnies", HttpStatus.NOT_MODIFIED);
    }
  }
