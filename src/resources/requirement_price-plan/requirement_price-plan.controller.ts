...
export class RequirementPricePlanController {
  constructor(private readonly requirementPricePlanService: RequirementPricePlanService) { }
  @Post()
  @UseGuards(RolesGuard)
  @Roles(Role.ADG, Role.PDG)
  async create(@Body() createChapterDto: Prisma.Price_plan_requirementCreateInput) {
    return this.requirementPricePlanService.create(createChapterDto);
  }


  @ApiResponse({ status: 200, description: 'successfully fetch all requirement price plan' })
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateChapterDto: Prisma.Price_plan_requirementUpdateInput) {
