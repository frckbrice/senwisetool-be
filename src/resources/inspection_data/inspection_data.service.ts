...
export class InspectionDataService {
  ...
  async findAll(query: { projectId: string, page?: number, perPage?: number }) {
    try {
      ...
      this.logger.error("can find all the inspection data", InspectionDataService.name);
      throw new NotFoundException("can find all the inspection data");
    }
  }

  async findOne(id: string) {
    return this.prismaService.inspection_data.findUnique({
      where: {
        id,
      }
    }).catch((err) => {
      this.logger.error("Can't find a inspection project data with id " + id, InspectionDataService.name);
      throw new NotFoundException("Can't find a projec data with id " + id);
    });
  }

  async update(id: string, updateInspectionDatumDto: Prisma.Inspection_dataUpdateInput) {
    return this.prismaService.inspection_data.update({
      where: {
        id,
      },
      data: updateInspectionDatumDto
    }).catch((err) => {
      this.logger.error("Can't update a inspection project data with id " + id, InspectionDataService.name);
      throw new NotFoundException("Can't update a inspection project with id " + id);
    });
  }
  ...
  async remove(id: string) {
    return this.prismaService.project.delete({
      ...
      this.logger.error("Can't delete a project with id " + id, InspectionDataService.name);
