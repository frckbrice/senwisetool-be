...
export class TrainingSessionService {
  ...
  async create(createTrainingSessionDto: Prisma.training_sessionCreateInput) {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientValidationError) {
        this.logger.error(`Error while creating training_session ${error.name}: Validation error \n\n ${error}`, TrainingSessionService.name);
        throw new InternalServerErrorException(`Validation Error while creating training_session ` + createTrainingSessionDto.id);
      }
      this.logger.error(`Error while creating training_session ${createTrainingSessionDto.id} \n\n ${error}`, TrainingSessionService.name);
      throw new InternalServerErrorException(`Error while creating training_session ` + TrainingSessionService.name);
    }
  }
  ...
  async findAll(query: Partial<PaginationTrainingSessionQueryDto>) {
    ...
    try {
      ...
      this.logger.error(`Error fetching training_sessions \n\n ${error}`, TrainingSessionService.name);
  ...
  async findOne(session_id: string) {
    ...
    try {
      ...
      this.logger.error(`Can't find a training with title: ${session_id} \n\n ${err}`, TrainingSessionService.name);
      throw new NotFoundException("Can't find a training with id " + session_id);
    };
  }

  async update(session_id: string, updateTrainingDto: Prisma.training_sessionUpdateInput) {
    ...
    try {
      ...
      this.logger.error(`Can't update a training_session  with id ${session_id} \n\n ${err} `, TrainingSessionService.name);
      throw new InternalServerErrorException("Can't update a training_session with session_id " + session_id);
    };
  }
  ...
  async remove(session_id: string) {
    try {
      ...
      this.logger.error(`Can't delete a training_session with session_id ${session_id} \n\n ${err}`, TrainingSessionService.name);
      throw new InternalServerErrorException("Can't delete a training_session with session_id " + session_id);
    };
  }
}
